...
exit;
*/
$sql="SELECT catNumber,Information FROM tis_releases";
$counter=0;
$res=mysql_query($sql);
while ($h=mysql_fetch_array($res)){
	$h['Information']=str_replace("\"","&quot",$h['Information']);
	$sql="UPDATE products set full_description = \"". $h['Information'] . "\" WHERE products.catalogue_number = \"" . $h['catNumber'] . " AND products.full_description = \"\"\"";
