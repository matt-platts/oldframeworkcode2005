    	...
    	if (p_value == '&nbsp;') p_value = '';
	  	if (l_isEditableColumn){
		  	if (p_columnModel.editableFieldType == "checkbox") {				
				...
				l_input.store('noResize', true);
				var l_width_offset_x = l_input.getStyle('padding-left').toInt() +
							 l_input.getStyle('padding-right').toInt() + 
							 l_input.getStyle('margin-left').toInt() +
							 l_input.getStyle('margin-right').toInt() + 
							 l_input.getStyle('border-left-width').toInt() +
							 l_input.getStyle('border-right-width').toInt();
				l_input.setStyle('margin-left', ((p_columnModel.width - l_width_offset_x)/2).toInt()); 
				...
				var l_width_offset = l_input.getStyle('padding-left').toInt() +
							 l_input.getStyle('padding-right').toInt() + 
							 l_input.getStyle('margin-left').toInt() +
							 l_input.getStyle('margin-right').toInt() + 
							 l_input.getStyle('border-left-width').toInt() +
							 l_input.getStyle('border-right-width').toInt();
				l_input.setStyle('width', p_columnModel.width - l_width_offset); 
    ...
    onCellResizeHandler: function(p_group, p_oldWidth, p_newWidth){
		...
		if ($chk(l_inputObj)){
			if (l_inputObj.retrieve('noResize')){
				l_widthOffset = (l_widthOffset/2).toInt();
	...
	toggleEditableCell: function(p_groupNode){
		...
		if ($chk(input)){	
			...
			if (input.hasClass('visible')){
				...
				input.fireEvent('removeCalendar');
			}
