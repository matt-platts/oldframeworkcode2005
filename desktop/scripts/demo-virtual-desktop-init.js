...
initializeWindows = function(){
	...
	MUI.dynMenuWindow = function(menuID,menuHref,menuText){
	...
	MUI.database_administratorWindow = function(){
		new MUI.Window({
			...
			loadMethod: 'iframe',
			contentURL: '../../mui-administrator.php?iframe=1&jx=1&dbf_mui=1',
			width:610,
			height:405,
			x:150,
			y:100
	...
	MUI.techsupportWindow = function(){
		new MUI.Window({
			id: 'tech_support_window',
	...
	MUI.pixlrWindow = function(){
		new MUI.Window({
			id: 'pixlrwindow',
			title: 'Image Editor',
			loadMethod: 'iframe',
			contentURL: 'http://www.pixlr.com/editor/',
			width:990,
			height:695,
			x:150,
			y:100
	...
	MUI.database_productsWindow = function(){
		new MUI.Window({
			...
			loadMethod: 'iframe',
			contentURL: '../../mui-administrator.php?action=display_content&content=178&iframe=1&jx=1&dbf_mui=1',
			width:240,
			height:590,
			x:950,
			y:100
	...
	MUI.database_administratorFullWindow = function(){
		new MUI.Window({
			id: 'database_administrator',
			title: 'Database Administrator',
			loadMethod: 'iframe',
			contentURL: '../../mui-administrator.php',
			width:1010,
			height:520
		});
	}
	...
	MUI.imagesWindow= function(){
		new MUI.Window({
			id: 'image_manager',
			title: 'Image Manager',
			loadMethod: 'iframe',
			contentURL: '../../mui-administrator.php?action=directory_browser&dir=./images&iframe=1&jx=1',
			width:1000,
			height:500
		});
	}
	...
	if ($('databaseadminLinkCheck')) {
		$('databaseadminLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.database_administratorWindow();
			MUI.database_productsWindow();
		});
	}
	if ($('databaseadminFullLinkCheck')) {
		$('databaseadminFullLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.database_administratorFullWindow();
		});
	}
	if ($('webbrowserLinkCheck')) {
		$('webbrowserLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.browserWindow();
		});
	}

	if ($('techsupportLinkCheck')) {
		$('techsupportLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.techsupportWindow();
		});
	}

	if ($('pixlrLinkCheck')) {
		$('pixlrLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.pixlrWindow();
		});
	}

	if ($('mydesktopLinkCheck')) {
		$('mydesktopLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.minimizeAll();
		});
	}
	if ($('usersLinkCheck')) {
		$('usersLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.dynMenuWindow("Users Window","../mui-administrator.php?action=list_table&t=user","User Manager");
		});
	}
	if ($('filemanagerLinkCheck')) {
		$('filemanagerLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.dynMenuWindow("File Manager","../mui-administrator.php?action=directory_browser&dir=.&dbf_mui=1","File Manager");
		});
	}


	if ($('imagesLinkCheck')) {
		$('imagesLinkCheck').addEvent('click', function(e){
		new Event(e).stop();
			MUI.imagesWindow();
		});
	}
