...
?>
/* 
	Set up windows, columns and panels, and then inititialize MUI.
	At the bottom of Core.js you can setup lazy loading for custom plugins.
*/
	...
	for (var i = 0; i < len; i++){
	  ...
	  if (thisDiv.className == 'dynMenuItem'){
		...
		$(thisDivID).addEvent('click', function(e){
			...
			// window name
			var pattern=/dbf_mui_wn=([\w|\s|%]+)/; // the % is for spaces ie %20
			winText=this.text;
			if (thisHref.match(pattern)){
					winText=RegExp.$1;
					winText=winText.replace("%20"," ");
			}

			// window size
			var pattern=/dbf_mui_ws=(\d+x\d+)/;
			if (thisHref.match(pattern)){
			...
			MUI.dynMenuWindow(this.id,this.href,winText,winWidth,winHeight);
		});
	  }
	}
...
window.addEvent('load', function(){
	...
	new WindowPicker({
		...
		onClose: function(w, e) {
			if ($defined(e)) { //console.log(w, e); // bug duly found in 5 minutes :/
				//MochaUI.notification('Event: Selection done!');
			}
		}
	});
...
var draggableOptions = {
    onStart:function()
    {
      // deprecated this.el.setOpacity(.5);
    },
    onComplete:function()
    {
