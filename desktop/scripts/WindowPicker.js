...
var WindowPicker = new Class({
	...
	action: function(e) {
		...
		for (var i = 0; i < this.e.length; i++) {
			...
			// mattplatts - the mochaui code to pull the window to the front
			if (j==0){ 
				MochaUI.focusWindow(document.getElementById(this.e[i].id));
			}
		}
	},
	
	updateElement: function(e, t, end) {
		...
		if (this.options.instant){ 
			...
			this.tweenOptions={duration:400};
			e.get('morph', this.options.tweenOptions).start(s);
	...
	start: function() {
		...
		//console.log("pos: " + this.position + " - " + document.getElementById(this.e[this.position].id).id);
