<?php
	...
	function setup_saveloadlist() {
		...
		//define("USER","medicodev2user");
		//define("PASSWORD","medicodev2pass");
		//define("DB","medicodev2");
		define("TABLE","wwwsqldesigner");
	}
	function setup_import() {
		//define("SERVER","localhost");
		//define("USER","globaldb");
		//define("PASSWORD","paragon555");
	...
	function import() {
		...
		$result_query="SELECT * FROM TABLES WHERE TABLE_SCHEMA = '".$db."'";
		$result = $db->query($result_query);
		$rowcounter=0;
		...
		$arr[] = $xml;
		$arr[] = '</sql>';
		file_put_contents("/var/www/vhosts/paragon-digital.net/httpdocs/medico_dev2/myxml.xml",implode("\n",$arr));
		return implode("\n",$arr);
	}
	...
	switch ($a) {
		case "list":
			...
			$result = $db->query("SELECT keyword FROM ".TABLE." ORDER BY dt DESC");
		...
		case "save":
			...
			$r = $db->query("SELECT * FROM ".TABLE." WHERE keyword = '".$keyword."'");
			if (mysql_num_rows($r) > 0) {
				$res = $db->query("UPDATE ".TABLE." SET data = '".$data."' WHERE keyword = '".$keyword."'");
			} else {
				$res = $db->query("INSERT INTO ".TABLE." (keyword, data) VALUES ('".$keyword."', '".$data."')");
			}
			if (!$res) {
		...
		case "load":
			...
			$keyword = (isset($_GET["keyword"]) ? $_GET["keyword"] : "");
			$keyword = mysql_real_escape_string($keyword);
			$result = $db->query("SELECT `data` FROM ".TABLE." WHERE keyword = '".$keyword."'");
