...
SQL.IO.prototype.init = function(owner) {
	...
	var ids = ["saveload","clientsave","clientload","clientsql","serversave","serverload","serverlist","serverimport","dbfimportrelations","dbfsaverelations"];
	...
	OZ.Event.add(this.dom.dbfimportrelations, "click", this.bind(this.dbfimportrelations));
	OZ.Event.add(this.dom.dbfsaverelations, "click", this.bind(this.dbfsaverelations));
	this.build();
}
...
SQL.IO.prototype.finish_and_save = function(xslDoc){
	...
	url = "http://www.paragon-digital.net/medico_dev2/mui-administrator.php?action=save_relations&pureAjax=1";
	var h = {"Content-type":"application/xml"};
	sendData="relations="+sql;
	OZ.Request(url, this.finishandsaveresponse, {xml:true, method:"post", data:sendData, headers:h});
	alert("Table relations saved");
}
...
SQL.IO.prototype.serverimport = function(e) {
	...
	alert("Calling import with " + name);
	this.owner.window.showThrobber();
	OZ.Request(url, this.importresponse, {xml:true});
}

SQL.IO.prototype.dbfimportrelations= function(e){
	var bp = this.owner.getOption("xhrpath");
	var url = "http://www.paragon-digital.net/medico_dev2/mui-administrator.php?action=visualise_relations&pureAjax=1";
	this.owner.window.showThrobber();
	OZ.Request(url, this.importresponse, {xml:true});
}

SQL.IO.prototype.dbfsaverelations= function(e){
        //var xml = this.owner.toXML();
        //this.dom.ta.value = xml;

	var bp = this.owner.getOption("staticpath");
	var path = bp + "db/"+window.DATATYPES.getAttribute("db")+"/output_relations.xsl";
	this.owner.window.showThrobber();
	OZ.Request(path, this.bind(this.finish_and_save), {xml:true});
}
...
SQL.IO.prototype.finishandsaveresponse = function(data, code) {
	this.owner.window.hideThrobber();
}
/* --------------------- table manager ------------ */
