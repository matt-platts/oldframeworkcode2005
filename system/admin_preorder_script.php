...
(paypal_preorder_reminder_sent IS NULL OR paypal_preorder_reminder_sent = \"0000-00-00 00:00:00\" OR paypal_preorder_reminder_sent < NOW() - INTERVAL 3 DAY) AND 
	...
	(preorder_auth_fail IS NULL OR preorder_auth_fail=\"0000-00-00 00:00:00\" OR 

	(preorder_auth_fail < NOW() - INTERVAL 3 DAY AND 
	(sagepay_auth_attempts<6 OR sagepay_auth_attempts IS NULL))) 
...
while ($h=$db->fetch_array($rv)){
	...
	if ($sagepay_payment_result['value']==1){
		...
		if ($act){
			...
			$update_order_sql="UPDATE orders set preorder_date_shipped = NOW(), complete = 1, paid=1, date_paid=NOW()  WHERE id = " . $details['order_number'];
			$rv=$db->query($update_order_sql);
			send_status_email("paypal_auth_and_send",$details['order_number'],$h['sagepay_auth_attempts']);
	...
	/* 2 - Paypal did not pay in advance and the customer needs a message... */
		...
		if ($act){
			...
			send_status_email("paypal_message",$details['order_number'],$h['sagepay_auth_attempts']);
	...
	/* 4 - Sagepay auth failed */
		...
		if ($act){
			...
			mail ("mattplatts@gmail.com","Auth attempts for " . $h['order_number'],"Made $sp_auth for " . $h['order_number']);
	...
	/* 5 - The final else - no boxes were ticked, this should not happen */
		...
		send_status_email("Unknown",$details['order_number'],$h['sagepay_auth_attempts']);
		$sp_auth=$h['sagepay_auth_attempts'];
		if (!$sp_auth){ $sp_auth=1;} else { $sp_auth++;}
		$updsql="UPDATE orders set preorder_auth_fail = NOW(), sagepay_auth_attempts=$sp_auth  WHERE id = " . $h['order_number'];
		$updrv=$db->query($updsql);
		...
		$action_taken['status']=1;
		mail ("mattplatts@gmail.com","Unknown problem found for " . $h['order_number'],"Made $sp_auth for " . $h['order_number']);
	}
...
function send_paypal_email($order_id,$attempts){
	...
	if ($act){
		...
		mail ("mattplatts@gmail.com","Auth attempts for " . $order_id,"Made $attempts for " . $order_id);
...
function send_status_email($action,$order_number,$auth_attempts_so_far){
	global $admin_email;
	global $preorders_email;
	global $short_name;
	if ($action=="paypal_message"){
		$subject="Pre Order #".$order_number. " has been messaged to the customer.";
		$message="Order number " . $order_number . " has been successfully messaged to the customer. This is attempt number $auth_attempts_so_far";
		...
		$message="Order number " . $order_number . " has been unsuccessfully authorised by Sagepay and not sent to flight logistics. This is attempt number $auth_attempts_so_far";
	}
...
function send_sagepay_auth_fail_message($details,$h){
	...
	$message .= "If your card is no longer valid or you wish to try again manually, you can enter your card details or select an alternative method of payment for this order at <a href=\"$weksite/load-order/".$details['order_number'] . "/\">$website/pre-order-paypal/159/</a><br /><br />Thanks,<br />$short_name Web Admin.<br /><br />";
