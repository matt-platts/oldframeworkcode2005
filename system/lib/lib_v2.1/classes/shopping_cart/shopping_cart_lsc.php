...
private function product_title_column_number(){
...
private function cart_contents_from_session(){
...
/*
	...
	$_SESSION['total_product_price']=sprintf("%4.2f",$running_total);
	return $return;
*/
}

private function total_items_in_cart(){
	foreach ($_SESSION['cart'] as $item => $itemdata){
		$counter++;
	}
...
public function add_to_preorder_cart($productid,$quantity){
	if ($_SESSION['order_number_set'] && !$_SESSION['allow_add_once']){
		$return_content=$this->unable_to_edit_order();
		print $return_content;
		exit;
...
public function update_preorder_cart(){
	if ($_SESSION['order_number_set'] && !$_SESSION['allow_add_once']){
		$return_content=$this->unable_to_edit_order();
		print $return_content;
		exit;
...
public function remove_from_preorder_cart($productid){
...
public function get_category_ids_as_array($current_category){
	$parent_category=$current_category;
	$parents=array();
	$i=0;
	while ($parent_category) {
...
public function get_parent_category($current_category){
	global $db;
...
public function display_mysql_cart_error($->db_error_message){
        print "An error has occurred in the mysql: $->db_error_message";
        return 1;
}

public function order_and_browse_buttons(){
...
public function view_cart_general($cart_view_options){
	global $db;
	$cart_template = $this->value("cart_template"); /* the MASTER template in which the cart display should sit */
	$default_currency_symbol = $this->value("default_currency_symbol");

	$cart_header=""; /* $cart_header is the header html of the cart, which contains the form opening html if allow_update is true, and the open table tag */
	...
	/* $cart_header_row is the top row of the table - no longer required for a templated cart */
	...
	$have_products=0;
	$trbg="#f1f1f1";
	$cart_data=array(); /* all the individual product line data goes into the $cart_data hash and is passed to the inner template */
	...
	foreach ($_SESSION['cart'] as $item => $itemdata){
		...
		while ($hash = $db->fetch_array()){
			if ($hash['vatable']){
				...
				$cart_data[$cart_lines_counter]['unit_price_inc_vat']=$default_currency_symbol . sprintf("%4.2f",($h['price_ex_vat']+$h['vat']));
			} else {
				$this_item_actual_vat=0;
				$cart_data[$cart_lines_counter]['vat_amount']=$default_currency_symbol . "0.00";
				$cart_data[$cart_lines_counter]['unit_price']=$default_currency_symbol . sprintf("%4.2f",($h['price_ex_vat']));
				$cart_data[$cart_lines_counter]['unit_price_inc_vat']=$default_currency_symbol . sprintf("%4.2f",($h['price_ex_vat']));
		...
		$running_total = $running_total + $line_cost;
		$running_total_ex_vat = $running_total_ex_vat + $line_cost_ex_vat;
		$running_vat= $running_vat + $this_item_actual_vat+$_SESSION['cart'][$item]['quantity'];
	...
	/* SESSIONSTORE: Store the total price of all the products in total_price */
	$_SESSION['total_product_price']=sprintf("%4.2f",$running_total);
	...
	if ($cart_view_options['run_modules']){
		...
		foreach ($itemise_checkout_modules['data'] as $checkout_module_name=>$checkout_module_data){
				$checkout_module_data['checkout_itemisation_text']=str_replace("{=voucher_text}",$_SESSION['voucher_text'],$checkout_module_data['checkout_itemisation_text']);
		...
		$_SESSION['checkout_modules_add_to_total']=$itemise_checkout_modules['total_amounts']['amount_to_add_to_total'];
		$return .= "<tr><td colspan=\"5\" style=\"height:5px; border:0px;\"></td></tr>";

		$EXPORT['grand_total']=$this->value("default_currency_symbol") . $this->calculate_grand_total_before_post_modules();
		...
		if ($this->value("post_order_total_values") ){
			...
			$post_counter=0;
			$total_post_mods=count($itemise_checkout_modules_post_order_total['data']);
			
			foreach ($itemise_checkout_modules_post_order_total['data'] as $checkout_module_name=>$checkout_module_data){
				if ($checkout_module_name != "amount_to_add_to_post_total"){
					$checkout_module_data['checkout_itemisation_text']=str_replace("{=voucher_text}",$_SESSION['voucher_text'],$checkout_module_data['checkout_itemisation_text']);
					$checkout_module_data['checkout_itemisation_text']=str_replace("{=promo_text}",$_SESSION['promo_text'],$checkout_module_data['checkout_itemisation_text']);
					$module_text=$checkout_module_data['checkout_itemisation_text'];
					//$module_text=str_replace("{=voucher_text}",$_SESSION['voucher_text'],$module_text);
					print "ON " . $checkout_module_data['total'];
					if ($checkout_module_data['total']){
						$running_post_total += $checkout_module_data['total'];
						...
						$EXPORT[$checkout_module_name]=$checkout_module_data['total'];
						$EXPORT['checkout_modules_text'] .= $checkout_module_data['checkout_itemisation_text'] . "<br />";
			print_r($checkout_module_data);
						$EXPORT['checkout_modules_total'] .= $this->value("default_currency_symbol") . $checkout_module_data['total']. "<br />";
						if ($post_counter<$total_post_mods-1){
							$return .= "<tr><td colspan=\"7\"><hr></td></tr>";
							$return .= "<tr><td colspan=\"5\" align=\"right\"><b>Total:</b></td><td align=\"right\">&pound;".sprintf("%4.2f",$running_total+$running_post_total)."</td></tr>";
						} else {
							if ($checkout_module_data['itemise_as_payment']){ $final_one_as_payment=1;} else { $final_one_as_payment=0;}
						}
					}
					$post_counter++;
				} else {
			...
			$_SESSION['checkout_modules_post_add_to_total']=$itemise_checkout_modules_post_order_total['total_amounts']['amount_to_add_to_total'];
			$_SESSION['checkout_modules_add_to_print_total']=$itemise_checkout_modules_post_order_total['total_amounts']['amount_to_add_to_stored_grand_total'];

		$EXPORT['grand_total']=$this->calculate_grand_total();
		$EXPORT['final_post_total']=$this->calculate_remaining_payment();
		if ($EXPORT['final_post_total']<0){
			$EXPORT['final_post_total']=0;
			$_SESSION['total_for_further_payment']=0;
			$_SESSION['voucher_only_payment']=1;
		} else {
			unset($_SESSION['total_for_further_payment']);
			unset($_SESSION['voucher_only_payment']);
		}
		// was the last one a payment?
		if ($final_one_as_payment){
			if ($cart_view_options['email']){
				$final_total_text="Payment by Credit Card:";
			} else {
				$final_total_text="Total Outstanding:";
			}
			$return.= "<tr><td colspan=\"7\"><hr></td></tr><tr><td colspan=\"5\" align=\"right\"><b>$final_total_text </b></td><td align=\"right\">&pound;" . $EXPORT['final_post_total'] . "</td></tr>";
			if ($EXPORT['final_post_total']<=0){ 
				$return.= "<tr><td colspan=\"5\" align=\"right\" style=\"color:#cc0000\">Please note: Your gift voucher payment exceeds your order total.<br />We do not give change for the remaining balance.</td><td align=\"right\"></td></tr>";
			}
		} else {
			$return.= "<tr><td colspan=\"7\"><hr></td></tr><tr><td colspan=\"5\" align=\"right\"><b>Grand Total: </b></td><td align=\"right\">&pound;" . $EXPORT['grand_total'] . "</td></tr>";
		}
		

		} else {
			$_SESSION['checkout_payment_amount']=$_SESSION['grand_total'];
		}
...
public function view_preorder_cart($cart_view_options){
...
public function view_preorder_cart_separates($cart_view_options){
	$preorder=1; // used to notify shipping functions
	global $db;
	$cart_template = $this->value("cart_template");
	$default_currency_symbol = $this->value("default_currency_symbol");
...
public function print_cart_and_preorder_total($run_modules){
	$_SESSION['total_of_all_orders']=$_SESSION['total_product_price'] + $_SESSION['preorder_total_price'];
...
private function get_product_title($title_fields){
	global $db;
	$product_title_fields_array=explode(",",$this->value("product_title_fields"));
...
private function format_product_title($title){
	$title = preg_replace("/(\d+( |k)?)g$/i","<span style=\"font-variant:normal;\">$1g</span>",$title);
	return $title;
}


private function print_cart_for_email(){
	global $db;
	$default_currency_symbol = $this->value("default_currency_symbol");
	$return="";
...
public function confirm_order($account_code_validated){
	...
	if (count($all_shipping_modules)>1 && ($this->value("buy_requires_login") && $user->value("id"))){
		...
		global $libpath;
		$shipping_module_file=$libpath."/classes/".$all_shipping_modules[0].".php";
	...
	$promos_active=$db->db_quick_match("checkout_modules","active","key_name","discount_codes");
	if ($promos_active && !$_SESSION['promo_text'] && ($this->value("buy_requires_login") && $user->value("id"))){
		$promo_code_form = $db->db_quick_match("templates","template","dbf_key_name","promo_code_form");
		$promo_code_form=str_replace("{=cart_template}",$cart_template,$promo_code_form);
		$checkout_template['promo_codes']=$promo_code_form;
		$content .= $checkout_template['gift_vouchers'];
	}
	...
	if ((!$_COOKIE['login'] || !$user->value("id")) && $this->value("buy_requires_login")==1){
		...
		if (!$account_code_validated && $golden_account_active){
			...
			$checkout_template['golden_account'] .= "<table width=\"100%\" style=\"background-color:#444444\"><tr><td width=30><form style=\"display:inline; margin:0px; margin-left:30px; padding:0px;\" method=\"post\" action=\"site.php?action=enter_golden_account_code&amp;mt=$cart_template\">Account&nbsp;No:</td><td><input type=\"text\" name=\"promotional_code\"> <input type=\"submit\" value=\"Apply Code\"></form></td></tr></table><br /></p></div>";
...
public function place_order(){
	...
	$total_amount=$_SESSION['total_product_price'];	
	...
	if ($payment_method != "voucher_only" && !$_SESSION['voucher_only_payment']){
		...
		print "This is a voucher only payment";
	}
	...
	if ($this->value("buy_requires_login")==1 || ($this->value("buy_requires_login")==2 && !$_SESSION['checkout_without_login'])){
		$order_sql = "INSERT INTO orders (ordered_by,order_date,datetime,complete,paid,product_total,purchased_through_account,payment_method,shipping_total,grand_total,origin) values(".$ordered_by.",NOW(),NOW(),0,0,$total_amount,\"$promotional_code\",\"$payment_method\",".$log_shipping.",".$_SESSION['grand_total'].",\"$source_identifier\")";
	} else {
		$order_sql = "INSERT INTO orders (non_account_order,order_date,datetime,complete,paid,product_total,purchased_through_account,payment_method,shipping_total,grand_total,origin) values(".$non_account_ordered_by.",NOW(),NOW(),0,0,$total_amount,\"$promotional_code\",\"$payment_method\",".$log_shipping.",".$_SESSION['grand_total'].",\"$source_identifier\")";
	}
	...
	print "Got to the end of place order!!";
}

public function place_pre_orders(){
	...
	foreach ($_SESSION['preorder_cart'] as $item => $itemdata){
		...
		$start_transaction_sql=$db->query("BEGIN");
		if ($this->value("buy_requires_login")==1 || ($this->value("buy_requires_login")==2 && !$_SESSION['checkout_without_login'])){
			$order_sql = "INSERT INTO orders (ordered_by,order_date,datetime,complete,paid,product_total,purchased_through_account,payment_method,shipping_total,grand_total,origin,pre_order) values(".$ordered_by.",NOW(),NOW(),0,0,$this_preorder_total_amount,\"$this_preorder_promotional_code\",\"$payment_method\",".$this_preorder_shipping.",".$this_preorder_grand_total.",\"$source_identifier\",1)";
		} else {
			$order_sql = "INSERT INTO orders (non_account_order,order_date,datetime,complete,paid,product_total,purchased_through_account,payment_method,shipping_total,grand_total,origin,pre_order) values(".$non_account_ordered_by.",NOW(),NOW(),0,0,$this_preorder_total_amount,\"$this_preorder_promotional_code\",\"$payment_method\",".$this_preorder_shipping.",".$this_preorder_grand_total.",\"$source_identifier\",1)";
		}
...
public function enter_promotional_code($promo_code){
	// lookup code
	global $db;
	global $user;
	...
	@list($code_validates,$voucher_errmsg)=$gv->process_enter_promotional_code($promo_code);
	...
	if (!$code_validates){
		if (!$voucher_errmsg){
			$voucher_errmsg="Sorry but this is not a valid voucher number. Please check the number and try again - Thank You.";
		}
		$return .= "<p class=\"dbf_para_alert\">$voucher_errmsg</p>";
		$return .= $this->confirm_order();
		return $return;
	} else {
		$code_validated=1;
		$return = "<p class=\"dbf_para_success\">Thank you - your voucher code has been validated. Your new order total is below.</p>";
		$return .= $this->confirm_order($promo_code);
		return $return;
	}
}

function enter_promotional_discount_code($promo_code){
	// lookup code
	global $db;
	global $user;

	require_once(LIBPATH . "/classes/shopping_cart/discount_codes.php");
	$gv = new discount_codes();
...
function calculate_sub_total(){
	$_SESSION['order_total'] = $_SESSION['total_product_price'] + $this->value("shipping_total");
	$_SESSION['grand_total'] = $_SESSION['total_product_price'] + $this->value("shipping_total");
...
function conslog($msg){
	...
	$msg=print_r($msg,1);
	$msg=str_replace("\"","",$msg);
	print "<script type=\"text/javascript\">console.log(\"$msg\");\n</script>";
	*/
}

function calculate_grand_total_before_post_modules(){
	$this->conslog("Calculating grand total before post modules: Got " . $_SESSION['grand_total']);
	$_SESSION['grand_total'] = sprintf("%4.2f",$_SESSION['total_product_price'] + $this->value("shipping_total"));
	$this->conslog("Calculating grand total before post modules: Got " . $_SESSION['grand_total']);
	if (preg_match("/[a-zA-Z]/",$this->value("shipping_total"))){
		$_SESSION['order_total'] = "";
	}
...
function calculate_grand_total(){
	...
	$this->conslog("got the following values: " . $_SESSION['grand_total'] . " " . $_SESSION['checkout_modules_add_to_print_total'] . " and " . $_SESSION['checkout_modules_add_to_total'] . " and " . $_SESSION['checkout_modules_post_add_to_total']);



	if ($_SESSION['checkout_modules_add_to_print_total']){
		$this->conslog("GOT THIS ONE - add " . $_SESSION['checkout_modules_add_to_print_total'] . " to the GT");
		$_SESSION['grand_total']=sprintf("%4.2f",$_SESSION['grand_total'] + $_SESSION['checkout_modules_add_to_print_total']);
	}
		$this->conslog("HA: " . $_SESSION['grand_total']);
	$_SESSION['stored_grand_total']=$_SESSION['grand_total'];
	$_SESSION['grand_total']=$_SESSION['grand_total'];
		$this->conslog("Grand total has been calculated as " . $_SESSION['grand_total']);
	return $_SESSION['grand_total'];
}

function calculate_remaining_payment(){
	$remaining_payment=$_SESSION['grand_total'];
		$this->conslog("GT here is $remaining_payment");
	
	if ($_SESSION['gift_vouchers_complex_total']){
		$this->conslog("Got a gvc total of " . $_SESSION['gift_vouchers_complex_total']);
		$remaining_payment=sprintf("%4.2f",$remaining_payment - $_SESSION['gift_vouchers_complex_total']); // because a gift voucher is ALWAYS a payment!
	}
	$this->conslog("RP is $remaining_payment");
	$_SESSION['checkout_payment_amount']=$remaining_payment;
	return $_SESSION['checkout_payment_amount'];
}
...
function print_cart_submit_form(){
	...
	$content .= "<form name=\"place_order_form\" method=\"post\" action=\"".HTTP_PATH."/site.php?s=$cart_default_web_site&action=place_order&mt=$cart_template\">\n";
...
function check_stock_quantity($product_id,$qty_ordered){
	...
	if (!$this->external_stock_check_function){
		...
		require_once("$libpath/classes/shopping_cart/external/$module.php");
	...
	if ($place_order_result==1){
		if (!$_POST['promotional_code_validated']){
			...
			if ($_SESSION['voucher_only_payment']){
				...
				print "This payment is covered by the oucher in full";
				$forwarding_page_text="<p><a href=\"/site.php?action=load_payment_success\">As no further payment is required, you can simply click here to complete your order.</a></p>";
			} else {
			...
			if ($module_specific_payment_function){
				if ($class_filename){
					...
					$class_filename = "$libpath/classes/shopping_cart/payment_modules/$class_filename";
...
function complete_order_after_payment_taken($order_number){
	...
	if ($this->value("external_order_post_function")){
		...
		require_once("$libpath/classes/shopping_cart/external/$post_module.php");
		...
		foreach ($all_codes as $each_code){
			...
			if ($extra_module){
				if ($extra_module != "this"){
					require_once("$libpath/classes/shopping_cart/$extra_module.php");
	...
	if (!$order_number){ // order number supplied means going through the new sagepay wrapper
		$cart_vars_to_unset=array("cart","total_product_price","order_id","grand_total","shipping_service","user_selected_shipping_service","payment_method_in_user","gift_vouchers_complex_code","gift_vouchers_complex_total","voucher_text","order_number_set","gift_vouchers_complex_type","gift_vouchers_complex","total_for_further_payment","voucher_only_payment","total_price","shipping_amount","vat_itemisation_total","checkout_modules_add_to_total","store_grand_total_without_payments","delivery_postcode","stored_grand_total","order_total","promo_text","promo_code","promo_discount_vat_by","payment_method_in_use","checkout_modules_post_add_to_total","checkout_modules_add_to_print_total","checkout_without_login","cart_vars","discount_codes_total","checkout_payment_amount");
		foreach ($cart_vars_to_unset as $cart_var){
			unset($_SESSION[$cart_var]);
		}
	}
...
function complete_order_after_payment_taken_new($order_number,$preorder){
	...
	if (!$preorder){ // order number supplied means going through the new sagepay wrapper
		unset($_SESSION['cart']);
		unset($_SESSION['total_product_price']);
...
function mail_final_order_confirmation_to_customer($order_number){
	...
	$print_order_detail=preg_replace("/src ?= ?\"(\w+) (\w+).jpg\"/","src=\"$1%20$2.jpg\"",$print_order_detail); /* spaces in image names */
...
function create_download_urls(){
	...
	while ($h=$db->fetch_array($res)){
		...
		$insert_url_res=$db->query($insert_download_sql) or die("Cant run $insert_download_sql: " . ->db_error());
...
function run_checkout_modules(){
	...
	$add_to_total=0;
	global $db;
	$sql="SELECT name,key_name,checkout_itemisation_text,class_file,front_end_display from checkout_modules WHERE itemisation_position = \"pre_grand_total\" AND active=1 ORDER BY ordering";
	$rv=$db->query($sql);
	while ($h=$db->fetch_array($rv)){
		$class_file=LIBPATH."/classes/shopping_cart/checkout_modules/".$h['class_file'];
		if ($_SESSION[$h['key_name']]['itemise_as_part_payment']){ $this->set_value("post_order_total_values",1); continue; } 
		if ($h['itemisation_position']=="post_grand_total"){ continue;}
		if (!$h['key_name']){ continue; }
		require_once($class_file);
		$checkout_mod_class = new $h['key_name'];
		$total = $checkout_mod_class->itemise_at_checkout(); 

		if ($h['front_end_display']){
			if (method_exists($checkout_mod_class, "post_text")){
				if ($checkout_mod_class->post_text()){
					$module_totals['data'][$h['key_name']]['post_text']=$checkout_mod_class->post_text();
				}
			}
			...
			if ($total && $total != 0 || ($checkout_mod_class->value("always_display_if_post_text"))){ // this hides any module with a value of 0
				...
				$module_totals['data'][$h['key_name']]['total']=sprintf("%4.2f",$total);
				$module_totals['data'][$h['key_name']]['name']=$h['name'];
				$module_totals['data'][$h['key_name']]['checkout_itemisation_text']=$h['checkout_itemisation_text'];
	...
	$module_totals['total_amounts']['amount_to_add_to_total']=$add_to_total;
...
function run_checkout_modules_post(){
	...
	$add_to_total=0;
	global $db;
	$sql="SELECT name,key_name,checkout_itemisation_text,class_file,itemisation_position FROM checkout_modules WHERE itemisation_position = \"post_grand_total\" AND active=1 ORDER BY ordering";
	$rv=$db->query($sql);
	while ($h=$db->fetch_array($rv)){
		$class_file=LIBPATH."/classes/shopping_cart/checkout_modules/".$h['class_file'];
		if ($h['itemisation_postion'] == "pre_grand_total"){ continue;}
		//if (!$_SESSION[$h['key_name']]['itemise_as_part_payment'] || $h['itemisation_position'] != "post_grand_total"){ print "Continuing on " . $h['class_file'] . "!"; continue; } 
		require_once($class_file);
		$checkout_mod_class = new $h['key_name'];
		$total = $checkout_mod_class->itemise_after_total(); 
		$this->conslog("On " . $class_file . " - itemise after");
		$ipp = $checkout_mod_class->value("itemise_as_part_payment");
		if (method_exists($checkout_mod_class, "post_text")){
			$module_totals['data'][$h['key_name']]['post_text']=$checkout_mod_class->post_text();
		}
		
		// check here for the -0 value that signifies a promo code activated but nothing to discount 	
		if (($total && $total != 0) || ($checkout_mod_class->value("always_display_if_post_text"))){ // this hides any module with a value of 0
			if (method_exists($checkout_mod_class, "post_text")){
				$module_totals['data'][$h['key_name']]['post_text']=$checkout_mod_class->post_text();
			}
			...
			$module_totals['data'][$h['key_name']]['name']=$h['name'];
			$module_totals['data'][$h['key_name']]['checkout_itemisation_text']=$h['checkout_itemisation_text'];
			$module_totals['data'][$h['key_name']]['itemise_as_payment']=$ipp;
			if ($checkout_mod_class->value("add_to_total")){
				$add_to_total = $add_to_total + $total;
			}
			if ($checkout_mod_class->value("add_to_stored_grand_total")){
				if (!$ipp){
					$add_to_stored_grand_total = $add_to_stored_grand_total + $total;
				}
			}
		}
		if (!$total || $total==0){ unset($module_totals['data'][$h['key_name']]);}
	}
	$module_totals['total_amounts']['amount_to_add_to_post_total']=$add_to_total;
	$module_totals['total_amounts']['amount_to_add_to_stored_grand_total']=$add_to_stored_grand_total;
	if ($add_to_total !=0 && $add_to_total != ""){ $this->set_value("post_order_total_values",1);}
	return $module_totals;
}
...
function get_best_shipping_rate($shipping_options){
	...
	foreach ($all_shipping_modules as $each_shipping_module){
		...
		$shipping_module_file=$libpath."/classes/shopping_cart/shipping_modules/".$each_shipping_module.".php";
		if (file_exists($shipping_module_file)){
			include_once($shipping_module_file);
...
function load_specific_shipping_module($user_set){
	global $libpath;
	$shipping_module=$libpath."/classes/shopping_cart/shipping_modules/".$user_set; // compare to db entries first!
...
function record_payment(){
	...
	if ($payment_method!="voucher_only"){
		$payment_method=$db->field_from_record_from_id("payment_modules",$payment_method,"key_name");
		...
		if ($record_payment){
			$sql="INSERT INTO payments (user,description,payment_amount,payment_date,order_number) VALUES(".$user->value("id").",\"Web Order - Immediate Payment via Credit Card - Order #".$_SESSION['order_id']." \",".$_SESSION['checkout_payment_amount'].",NOW(),".$_SESSION['order_id'].")";
			$rv=$db->query($sql);
		}
	} else {
		if ($record_payment){
			$sql="INSERT INTO payments (user,description,payment_amount,payment_date,order_number) VALUES(".$user->value("id").",\"Web Order - Full Payment via Voucher - Order #".$_SESSION['order_id']." \",".$_SESSION['checkout_payment_amount'].",NOW(),".$_SESSION['order_id'].")";
			$rv=$db->query($sql);
		}
	}
}
...
function escape_pre_order_payment(){
	...
	unset($_SESSION['grand_total']);
	unset($_SESSION['checkout_payment_amount']);
...
function cancel_promo_code(){
	require_once(LIBPATH . "/classes/shopping_cart/discount_codes.php");
	$checkout_mod_class = new discount_codes; 
	$checkout_mod_class->clear_down_promo_codes();
	header("Location: checkout.html");	
	exit;
}
