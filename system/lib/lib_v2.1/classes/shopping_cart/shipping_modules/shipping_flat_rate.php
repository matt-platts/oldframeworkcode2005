...
function calculate_shipping_rate($shipping_options){
	...
	if ($buy_requires_login && !$user->value("id")){ return "Please log in for a shipping quote"; }
	if (!$buy_requires_login && $country_if_no_login){
	...
	// product attributes may kill this..

	/*if ($delivery_country_id==182){
		$initial=$this->value("initial_rate_uk");
		$additional=$this->value("additional_item_uk");
	} else if ($continent=="Europe"){
		$initial=$this->value("initial_rate_europe");
		$additional=$this->value("additional_item_europe");
	} else if ($continent=="North America"){
		$initial=$this->value("initial_rate_usa");
		$additional=$this->value("additional_item_usa");
	} else {
		$initial=$this->value("initial_rate_row");
		$additional=$this->value("additional_item_row");
	}*/
	...
	$all_cart_items=$priority_shipping_rate_items;
	foreach ($standard_shipping_rate_items as $cartitem => $cartvalues){
		$all_cart_items[$cartitem]=$cartvalues;
	}
	//print_debug($standard_shipping_rate_items);	
	//print_debug($priority_shipping_rate_items);	
	//print_debug("<hr>");
	//print_debug($all_cart_items);
