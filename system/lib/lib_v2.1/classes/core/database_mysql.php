...
function __construct($database=""){
	...
	$this->dbh = mysql_connect($this->host,$this->username,$this->password) or die("ERROR M1BV: Could not connect to database");
	$this->selected = mysql_select_db($this->database) or die("ERROR M2BV");
	return $this->dbh;
}
...
public function query($sql){
	...
	$this->result = mysql_query($sql) or format_error("ERROR 5198180 in $sql", $this->db_error_code(), "", mysql_error() . ".<br />Originating Statement: " . $sql);
	$this->query_counter++;
...
public function user_content_query($sql){
	...
	$this->result = mysql_query($sql) or format_error("ERROR 519818UC",1,"",mysql_error() . ".<br />Originating Statement: " . $sql . " [END]");
	$this->query_counter++;
	//print "Query count: " . $this->query_counter . ": $sql<br />";
	return $this->result;
...
public function admin_update_query($sql,$table,$rowid){
	...
	$this->result = mysql_query($sql) or format_error("ERROR 519818",$this->db_error_code(),"",mysql_error() . ".<br />Originating Statement: " . $sql);
	$this->query_counter++;
 ...
 * Meta: Maps onto PHPs mysql_fetch_array
*/
...
public function record_from_id($from_table,$record_id){
	...
	$from_table 	 = mysql_real_escape_string($from_table);
...
public function db_quick_match($from_table,$return_field,$condition_field,$condition_value){
	...
	$from_table 	 = mysql_real_escape_string($from_table);
	$return_field 	 = mysql_real_escape_string($return_field);
	$condition_field = mysql_real_escape_string($condition_field);
	$condition_value = mysql_real_escape_string($condition_value);
...
public function field_from_record_from_id($from_table,$record_id,$table_field,$where=null){
	...
	$sql = "SELECT " . $table_field . " FROM " . $from_table . " WHERE id = " . $record_id;
	...
	$get_record_result=$this->query($sql) or format_error("Error Code 81787 " . mysql_error(),1,"","$sql generated the error " . mysql_error());
 ...
 * Dev note: Do not attempt to change mysql_query to db->query in the lines below or you will cause an infinite permissions loop!
*/
private function check_permissions($tablename, $permission_type, $permission_rowid, $try_as_admin=0){
	global $user;
	...
	$permissions_result=mysql_query($permissions_sql);
	$permissions_on_table=array();
	while ($permissions_row = mysql_fetch_array($permissions_result,MYSQL_ASSOC)){
		...
		if ($user->value("id")){
			...
			if ($table_permission['value']=="current_user()"){
				...
				if ($permission_type != "add_or_edit" || ($permission_type=="add_or_edit" && $permission_rowid)){
					...
					while ($check_sql_row=mysql_fetch_array($check_result,MYSQL_ASSOC)){
...
/* 
 * Function db_escape
 * Meta: public method to mysql_real_escape_string
*/
...
/* 
 * Function db_errmsg
 * Meta: public method to mysql_error
*/
 ...
 * Meta: public method to mysql_num_rows
 * Dev note: We are passing in result as per old mysql_ extension, but mysql is a bit different. Note that its currently not using this input.
*/
