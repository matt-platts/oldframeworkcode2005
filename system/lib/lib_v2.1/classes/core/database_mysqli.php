...
function __construct($database=""){
	...
	$this->selected = mysqli_select_db($this->dbh,$this->database) or die("ERROR M2BV");
	return $this->dbh;
}
...
public function query($sql){
	...
	$this->result = mysqli_query($this->dbh,$sql) or format_error("ERROR 519818i in $sql",$this->db_error_code(),"",mysqli_error() . ".<br />Originating Statement: " . $sql);
	$this->query_counter++;
...
private function check_permissions($tablename, $permission_type, $permission_rowid, $try_as_admin=0){
	...
	while ($permissions_row = mysqli_fetch_array($permissions_result,MYSQL_ASSOC)){
		...
		if ($user->value("id")){
			...
			if ($table_permission['value']=="current_user()"){
				...
				if ($permission_type != "add_or_edit" || ($permission_type=="add_or_edit" && $permission_rowid)){
					...
					while ($check_sql_row=mysqli_fetch_array($check_result,MYSQL_ASSOC)){
