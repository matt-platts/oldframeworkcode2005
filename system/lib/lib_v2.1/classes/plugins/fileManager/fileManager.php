...
class fileManager {
	...
	public function fileManagerFront(){
		ob_start();
		open_col2();
	...
	public function file_manager_main($directory,$list_type,$display_options,$options_position,$default_no_per_page,$list_dir_options){
		global $db;
		...
		if ($display_options && (!$options_position || $options_position=="top" )){
			...
			$option_list=csv_to_options($list_for_options,$selected_dir,$files_array);
	...
	public function list_directory($dir,$listtype,$default_no_per_page,$list_dir_options){
	...
	<?
		...
		if ($list_dir_options['default_title']){
			...
			echo "<span class=\"file_browser_title\">Showing Photos In: $sub_dir $directory_name. Click on the trash icon to delete a photograph.</span>";
		}
		...
		if (is_dir($dir)) {
			...
			while (($file = readdir($dh)) !== false) {
			    if ($file != "." && $file != ".." && !is_dir($dir/$file)){
				    ...
				    if ($listtype == "table"){
					   ...
					   if ($count==1){echo "<tr>";}
					   if ($list_dir_options['thumbnail_directory']){$display_image_dir=$list_dir_options['thumbnail_directory'];}else{$display_image_dir=$dir; }
			...
			format_error("$dir is not a directory. program terminating.",1);
		}
		if ($count != 0){echo "</tr>";}
		echo "</table>";
		echo "</form>";
	...
	//-->
	   ...
	   if (!$_GET['dir'] || $_GET['dir']=="./" || $_GET['dir']=="." || (!preg_match("/images/",$_GET['dir']) && !preg_match("/files/",$_GET['dir']) && !preg_match("/downloads/",$_GET['dir']) && !preg_match("/documents/",$_GET['dir']))){
		...
		print "<p><a href=\"javascript:history.go(-1)\">Back</a></p>"; $content=ob_get_contents(); ob_end_clean(); return $content;
		}
	...
	<?php 
		...
		print "<div style=\"float:right; display:inline; margin-top:6px; clear:none;\" align=\"right\">" . upload_file_form(str_replace("./","",$dir));
	...
	<?php
	   ...
	   ?> 
	    <tr>
		<td><img src="<?php print SYSIMGPATH;?>/icons/folder.png"></td>
	   ...
	   ?> 
		...
		<?php if ($graphic=="image" && count($fileArray)<300){
			...
			if (filesize($imgfile)<=500000){
				print "<img src=\"".preg_replace("/^\.\//","",$dir).$fileArray[$i]."\" width=\"15\" height=\"15\" />";
			} else {
			 print "<img src=\"".SYSIMGPATH."/icons/".$graphic.".png\">";
			}
		} else {
			 print "<img src=\"".SYSIMGPATH."/icons/".$graphic.".png\">";
		}?>
		...
		<a title="Preview" href="Javascript:void window.open('<?=$dir.$fileArray[$i];?>','previewWindow','width=500,height=450');"><img src="<?php print SYSIMGPATH;?>/icons/monitor.png" border="0"></a>  
		<a href="<?=$dir.$fileArray[$i];?>" class="mb" title="Preview in popup: <?=$dir.$fileArray[$i];?>"><img src="<?php print SYSIMGPATH;?>/icons/monitor_go.png" border="0"></a>
		<?php }  else { ?>
			<a title="Preview" href="<?=get_link($dir.$fileArray[$i],"Image Preview","",1);?>"><img src="<?php print SYSIMGPATH;?>/icons/monitor.png" border="0"></a>  
		<?php } 
		// edit link?	
		if ($include_edit){
			if ($graphic=="image"){
		...
		global $libpath;
		print " <a href=\"$libpath/library/html_to_word.php?document=".$fileArray[$i]."\" title=\"Download Word Document\"><img src=\"".SYSIMGPATH."/icons/page_white_word.png\" border=\"0\" /></a>";
		}
	?>
		<a href="Javascript:file_rename('<?php print $_SERVER['PHP_SELF'];?>','<?php print $fileArray[$i];?>','<?php print $dir;?>','<?php print $showpath;?>')" title="Rename" alt="Rename"><img src="<?php print SYSIMGPATH;?>/icons/control_play.png" border="0"></a>
		<a href="<?php print $deleteUrl;?>" title="Delete" alt="Delete"><img src="<?php print SYSIMGPATH;?>/icons_companion/control_remove.png" border="0"></a>
		</td>
	    </tr>
	    <?
	    }
	...
	<p>
	<input type="image" src="<?php print SYSIMGPATH;?>/application_images/save_beige_43x39.png" border="0"> 
	</p>
	</form>
	...
	public function directory_browser_delete_file($file){
	...
	public function select_file_manager_macros($directory){
		//return;
		global $db;
	...
	public function run_batch_macro($directory,$macro_id){
		global $db;
	...
	public function get_directory_list($dirname,$filetypes){
	...
	public function directory_browser_file_rename($dir,$old,$new){
