...
*/
class order_adminController extends baseController {

public function view_order(){
	ob_start();
	global $db;
	global $page;
	$orderno = $this->Routing['view_order'];
	...
	while ($h=$db->fetch_array($res)){
		...
		$res2=$db->query($sql) or die($db->db_error());
		while ($h2=mysql_fetch_array($res2)){
			...
			if ($db->num_rows($att_res)>=1){
				...
				while ($j=$db->fetch_array($att_res)){
