...
// We already have content from the require for view_cart - this needs to e uilt into something more cosmic...
...
// get params from url
$urlParts = array_splice($urlParts,3,sizeof($urlParts)); // remove base, controller and function
$params = array();
$i=0;
while ($i < sizeof($urlParts)){
...
if (method_exists($loadController,$loadAction)){
	$response = call_user_func(array($loadController,$loadAction),$params);
} else if ($debug) {
...
if (null === $response){
	...
	if (!$page->value("content")){
        ...
        // need to stuff the content into a template - this is taken care of in the multi action controller..
        $result = $page->page_setup($page->value("content"));
}


// all content is loaded into the view in the page class, we use the view class to print it
// view_cart is not yet templated - how dpo we do this??
$view->set_value("view","site.html");
