...
function dbf_tag_parse($string,$loop){
// This is the master function that deals with all dbf tags even if embedded!! Currently does not deal with the sql:endsql bits in templates though
	...
	foreach ($matches[0] as $thismatch){
		...
		$replace=$this->get_value_for_dbf_tag($replace,"");
		$string=str_replace($thismatch,$replace,$string);
		//print $string . "\n";
	}
	if (stristr($string,"{=")){ $loop++; $string = $this->dbf_tag_parse($string,$loop); $loop--; }
	return $string;
}

function get_value_for_dbf_tag($string,$data){
	$return=$string;
	...
	if (stristr($string,"SQL:")){$string=preg_replace("/^SQL:/","",$string); $return=$this->evaluate_sql($string);}
	if (stristr($string,"FORM:")){$return=$this->evaluate_form_in_content($string);}
	
	return $return;
}

function evaluate_sql_in_content($sent_content){
	global $user;
	global $db;
	if ($_GET['pageEdit']){return $this->internal_edit_links($sent_content);} // if inline editing, insert edit links for SQL instead
	$debug=0;
	if ($debug){print "<hr><h1>Reached eval_sql_in_content_function</h1><hr>";}
	$sent_content=$this->evaluate_form_in_content($sent_content);
	$sent_content=$this->evaluate_recordset_in_content($sent_content);
	$sent_content = $this->parse_request_vars($sent_content);
	$sent_content = $this->parse_string_for_system_functions($sent_content);
	$sent_content = $this->get_file_manager_instances($sent_content);
	global $page;
	$sent_content = $page->load_template_widgets($sent_content);
        ...
        if (strpos($sent_content,"{=PHP:")){
		$sent_content=$this->php_in_content($sent_content);
        }
	if (strpos($sent_content,"{=sendtofunction:")){
                $split_content=explode("{=sendtofunction:",$sent_content);
		...
		$embed_external_output=$functionResult;
                $sent_content=str_replace($original_php,$embed_external_output,$sent_content);
	}

	$sql_matches=preg_match_all("/{=SQL:.*sql}/Uis",$sent_content,$matches);
	
	foreach ($matches[0] as $each_match){
		if ($debug){print "matched $each_match<br>";}
		...
		$get_query=preg_match("/{=SQL:.*}/Uis",$each_match,$querymatches);
		...
		if (!$tableresults[1]){ print "Error: No table found for table results in code parser. This could be to do with a line break in the query?<p>"; var_dump($sql_query); print "<p>"; var_dump($tableresults); exit;}
		...
		$result=$db->user_content_query($sql_query) or format_error("ERROR 9287B",0,"","Query:$sql_query<br />".->db_error());
		...
		foreach($rowdata as $returned_record_header => $returned_record_data){
			...
			$s=$sub_template;
			$r = preg_match_all("/{=if ((?:!?\w+,?)+ ?=? ?\w+)}(.*?){=end[ _]?if}/ims",$s,$or_results); // remove the ? after the , to stop capturing single if statements
			...
			//$r = preg_match_all("/{=if ([!?\w\++]+\w+)}(.*?){=end[ _]?if}/ims",$s,$and_results); // this was in lib and different from the below line which was native to v2.1
			...
			if ($debug){print "<p>ON FIELD $fieldname";}
				...
				if (stristr($fieldname,"PassSQLValueToFunction:")){
					...
					/* 6 lines below added to v2.1 later */
					if ($functionname=="strip_tags.php"){
						$strip_tags=1;
					}
					if ($functionname=="strip_tags_and_concat_description"){
						$strip_concat=1;
					}
					...
					if ($strip_tags){
						$returned_record_data[$fieldname]=strip_tags($string_to_function);
						$returned_record_data[$fieldname]=preg_replace("/^\"\" \"\" \"/","",$returned_record_data[$fieldname]);
					} else if ($strip_concat){
						$returned_record_data[$fieldname]=substr(strip_tags($string_to_function),0,300);
						$returned_record_data[$fieldname]=preg_replace("/^\"\" \"\" \"/","",$returned_record_data[$fieldname]);
					} else {
		...
		if (!$cancelrest){
			$sent_content=str_replace($each_match,$replacewith,$sent_content);
		} else {
			$sent_content=str_replace($each_match,"",$sent_content);
		}
		if ($cancelme){
			$cancelrest=1;
			$cancelme=0;
			$sent_content=preg_replace("/\<\!--begin cancel page ?\/\/--\>.*\<\!--end cancel page ?\/\/--\>/is","",$sent_content);
		}
	}

$sent_content = $this->parse_template_code($sent_content);
return $sent_content;
}

function internal_edit_links($sent_content){
	return $sent_content;
	global $db;
	$debug=0;
...
function evaluate_form_in_content($sent_content){
	...
	foreach ($matches[0] as $each_match){
		...
		if ($form_query){
			...
			$form_rowid=$this->dbf_tag_parse($form_rowid,0);		
			...
			if ($form_table && $form_type){
				...
				if (filter_registered_on_table($form_table)){
					$load_filter_options=$new_form_filter->load_filter(filter_registered_on_table($form_table));
				...
				$embedded_form=form_from_table($form_table,$form_type,$form_rowid,$form_add_data,$form_options);
...
function evaluate_recordset_in_content($rs_content){
	...
	foreach ($matches[0] as $each_recordset_match){
		...
		if ($recordset_filter && $recordset_source){
			...
			if (stristr($_SERVER['SCRIPT_NAME'],"administrator.php")){
				$inner_content = list_table($tablename,$dbforms_options);
			} else {
...
function sql_to_select_options($sql){
	global $db;
	$res=$db->query($sql);
	while ($h=$db->fetch_array($res)){
		$returnstring .= "<option value = \"".$h['id']."\">".$h['title']."</option>";
	}
	return $returnstring;
}

function parse_string_for_system_functions($stringin){
...
function get_file_manager_instances($content){
	global $db;
...
function parse_request_vars($string_in){
	$result=preg_match_all("/_GET\['\w+'\]/",$string_in,$matches);
	foreach ($matches[0] as $match){
		$match=str_replace("_GET['","",$match);
		$match=str_replace("']","",$match);
		$match_result=$db->db_escape($_GET[$match]);
		$string_to_match="{=$" . "_GET['" . $match . "']}";
		$string_in=str_replace($string_to_match,$match_result,$string_in);
		$string_to_match="$" . "_GET['" . $match . "']";
		$match_result=str_replace("\\'","'",$match_result);
		$string_in=str_replace($string_to_match,$match_result,$string_in);
	}
	$result_req=preg_match_all("/_REQUEST\['\w+'\]/",$string_in,$matches_req);
	foreach ($matches_req[0] as $match){
		$match=str_replace("_REQUEST['","",$match);
		$match=str_replace("']","",$match);
		$match_result=$db->db_escape($_REQUEST[$match]);
		$string_to_match="{=$" . "_REQUEST['" . $match . "']}";
		$string_in=str_replace($string_to_match,$match_result,$string_in);
		$string_to_match="$" . "_REQUEST['" . $match . "']";
		$match_result=str_replace("\\'","'",$match_result);
		$string_in=str_replace($string_to_match,$match_result,$string_in);
	...
	$result_req=preg_match_all("/_SESSION\['\w+'\]/",$string_in,$matches_req);
	foreach ($matches_req[0] as $match){
		$match=str_replace("_SESSION['","",$match);
		$match=str_replace("']","",$match);
		$match_result=$db->db_escape($_SESSION[$match]);
		$string_to_match="{=$" . "_SESSION['" . $match . "']}";
		$string_in=str_replace($string_to_match,$match_result,$string_in);
		$string_to_match="$" . "_SESSION['" . $match . "']";
		$match_result=str_replace("\\'","'",$match_result);
		$string_in=str_replace($string_to_match,$match_result,$string_in);
...
function global_vars($data){
	// replace all global vars with values
...
function php_in_content($input){
	...
	if (strpos($input,"{=PHP:")){
		$input=$this->php_in_content($input);
	}
	return $input;
}

function parse_php_in_query($input){
...
function parse_form_template_code($input,$data){
