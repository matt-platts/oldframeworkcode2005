...
class recordset {
	...
	protected $options;
		...
		if (preg_match("/^QUERY:/",$recordset_source)){
			...
			$queryresult=$db->query($query) or format_error("Error: 98889 - Cant run sql" . ->db_error(),1);
			global $codeparser;
			...
			$this->set_value("fields_to_display_from_query",list_selected_fields_from_query($this->value("actual_query")));
		}
...
function store_dynamic_display_fields(){
...
function store_dynamic_search_fields(){
...
function merge_recall_with_request_vars(){
...
function map_recall_and_request_onto_initial_filter(){
...
function delete_record_form($sitevar){
	...
	$this->delete_form = "<!-- delete from recordset object //-->\n";
        ...
        $this->delete_form .= "<input type=\"hidden\" name=\"relation_key\" value=\"".$_REQUEST['relation_key']."\">\n";
        $this->delete_form .= "<input type=\"hidden\" name=\"relation_id\" value=\"".$_REQUEST['relation_id']."\">\n";
...
function print_recordset_header(){
	// print table header
	$print_tablename=ucfirst(str_replace("_"," ",$this->value("recordset_source")));
	if ($_REQUEST['relation_id']){
		$master_table_for_navigation=get_master_table_from_relationship($this->value("recordset_source"),$_REQUEST['relation_id']);
		$master_table_for_navigation_print=ucfirst(str_replace("_"," ",$master_table_for_navigation));
		$print_tablename="<a href=\"".$_SERVER['PHP_SELF']."?s=1&action=list_table&t=$master_table_for_navigation&dbf_add=1&dbf_edi=1&dbf_ido=1&dbf_search=1&dbf_sort=1&dbf_sort_dir=1&dbf_orderby=1&dbf_rpp_sel=1&dbf_rpp=All\" style=\"text-decoration:underline\">$master_table_for_navigation_print</a> : " . $print_tablename;
	}

	if ($this->options['filter']['title_text']){ 
		$EXPORT['title_text'] = $this->options['filter']['title_text']; 
		$print_title=$this->options['filter']['title_text']; 
	} else { 
		$print_title = "You are in: " . $print_tablename;
	}
	...
	//if (!$options['filter']['append_section_name_to_breadcrumb']){
		...
		if ($_REQUEST['relation_id'] && $_REQUEST['relation_key']){
			...
			$child_table_key_field=get_child_key_field_from_child_table($this->value("recordset_source"),$_REQUEST['relation_id']);
			$master_table_key_field=get_master_key_field_from_child_table($this->value("recordset_source"),$_REQUEST['relation_id']);
			...
			$master_table_for_print_filter=get_master_table_from_relationship($this->value("recordset_source"),$_REQUEST['relation_id']);
			$mtsql="SELECT $master_table_key_field FROM $master_table_for_print_filter WHERE $master_table_key_field = ".$_REQUEST['relation_key'];
			global $db;
			$mtres=$db->query($mtsql) or die(->db_error());
			...
			if ($this->options['filter'][$child_table_key_field]['select_value_list']){
				$mth_result = sql_value_from_id($this->options['filter'][$child_table_key_field]['select_value_list'],$mth_result);
			} else {
...
function print_header_interface_buttons(){
	...
	if ($this->options['include_add_link'] || $this->options['filter']['include_add_link']){
		...
		if ($_REQUEST['relation_id'] && $_REQUEST['relation_key']){ $add_row_url .= "&relation_key=".$_REQUEST['relation_key']."&relation_id=".$_REQUEST['relation_id'];}
	...
	if ($this->options['include_edit_all_link'] || $this->options['filter']['include_edit_all_link']){
		if ($_REQUEST['filter_id']){$filter_text="&filter_id=".$_REQUEST['filter_id'];} else if (filter_registered_on_table($this->value("recordset_source"),"list_table")){$filter_text .= "&filter_id=".filter_registered_on_table($this->value("recordset_source"),"list_table");} else {$filter_text .= "&filter_id=";} 
		if ($_REQUEST['relation_id'] && $_REQUEST['relation_key']){
			 $relation_key_text = "&relation_key=".$_REQUEST['relation_key']."&relation_id=".$_REQUEST['relation_id'];
		}
		$edit_all_js_url = $_SERVER['PHP_SELF'] . "?action=edit_table&edit_type=edit_all&t=".$this->value("recordset_source")."&add_data=1" . $filter_text . $relation_key_text; 
...
function print_recordset_filtering(){
	...
	if ($this->options['dbf_sort'] || $this->options['dbf_filter'] || $this->options['dbf_rpp_sel'] || $this->options['dbf_search'] || $this->options['filter']['dbf_search'] || $this->options['filter']['dbf_filter'] || $this->options['filter']['dbf_sort']){
		$output_text .= "<div id=\"page_filters\"><div id=\"filters_div\"><p>\n";
	...
	if (!$this->value("on_query")){
		$search_fields_list=populate_search_fields($this->value("recordset_source"),$this->options['filter'],$this->filter_values['dbf_search_fields']);
	} else { 
		if ($this->options['filter']['search_fields']){
			$search_fields_list = populate_search_fields($this->actual_query,$this->options['filter'],$this->options['filter']['search_fields']);

		} else {
			$search_fields_list = queryfields_as_select_options($this->actual_query,$this->options['filter'],$_REQUEST['dbf_search_fields']);
		}
	}
		if ($_REQUEST['dbf_dynamic_fields_to_display_list']){
			$dyn_fields_to_display_list=$_REQUEST['dbf_dynamic_fields_to_display_list'];
		...
		if($dyn_search_fields_list){
			...
			$search_fields_list=populate_search_fields($this->value("recordset_source"),$this->options['filter'],$dyn_search_fields_list);
		}
...
<div class="collection">
<div class="item_value_top"><input type="text" name="dbf_search_for" value="'.$this->filter_values['dbf_search_for'] . '"></div><div class="item_value_bottom"><select name="dbf_search_fields">'.$search_fields_list.'</select><br /><a href="Javascript:dbf_search_popup_open(\''.$this->value('recordset_source').'\',\''. $this->options['filter']['filter_id'] . '\',\'' . $dyn_search_fields_list. '\')">-&gt;select fields</a></div>
</div>
</div>
...
$output_text .= "\n";
		...
		$EXPORT['search_input_field'] .= "<input type=\"text\" name=\"dbf_search_for\" size=\"10\" class=\"page_filter_text_input\" value=\"" . $this->filter_values['dbf_search_for'] . "\" onfocus=\"this.value='';\">\n";
		if ($this->options['filter']['dbf_search_fields_sel']){
			$EXPORT['search_input_field'] .= "<select name=\"dbf_search_fields\" class=\"page_filter_select\">".populate_search_fields($this->value("recordset_source"),$this->options['filter'],$_REQUEST['dbf_search_fields'])."</select>";
		} else {
	...
	if (!$this->value("on_query")){
		$filter_fields_list=populate_filter_fields($this->value("recordset_source"),$this->options['filter'],$this->filter_values['dbf_data_filter_field']);
	} else {
		if ($this->options['filter']['filter_fields']){
			$filter_fields_list=populate_filter_fields($this->value("recordset_source"),$this->options['filter'],$this->filter_values['dbf_data_filter_field']);
		} else {
			$selected_filter_field=str_replace("","",$this->filter_values['dbf_data_filter_field']);
			$filter_fields_list = queryfields_as_select_options($this->actual_query,$this->options['filter'],$selected_filter_field);
		}
	}
...
<div class="collection">
<div class="filter_item_text">Filter By:</div><div class="filter_item_text"><select name="dbf_data_filter_operator" class="small" style="width:35px">'. csv_to_options("=,&lt;,&gt;,IN",$current_operator) . '</select></div>
</div>
<div class="collection">
<div class="item_value_top"><select name="dbf_data_filter_field">' . $filter_fields_list . '</select></div><div class="item_value_bottom"><input type="text" name="dbf_data_filter_value" value="'. $this->filter_values['dbf_data_filter_value'] .'"><br />';
...
$output_text .= '</div>
	...
	if (!$this->value("on_query")){
		...
		$sort_fields_list=populate_sort_fields($this->value("recordset_source"),$this->options['filter'],$this->filter_values['dbf_sort_by_field']);
	} else { 
		if ($this->options['filter']['sort_fields']){
			...
			$sort_fields_list = queryfields_as_select_options($this->actual_query,$this->options['filter'],$_REQUEST['dbf_search_fields']);
		}
	}
...
<div class="item_value_top"><select name="dbf_sort_by_field">' . $sort_fields_list . '</select></div><div class="item_value_bottom">';
	...
	if ($this->options['dbf_sort_dir'] || $this->options['filter']['dbf_sort_dir']){$output_text .= '<select name="dbf_sort_direction">' . csv_to_options("Asc;;Ascending,Desc;;Descending",$this->filter_values['dbf_sort_direction']) . '</select>';}
 	$output_text .= '</div>
</div>
</div><!-- closes filter //-->';
		...
		$EXPORT['sort_selector'] .= ">" . populate_sort_fields($this->value("recordset_source"),$this->options['filter'],$this->filter_values['dbf_sort_by_field']) . "</select>";
	...
	*/
		...
		//	$output_text .= " <br clear=\"all\"/><select name=\"dbf_sort_direction\" class=\"page_filter_select\">" . csv_to_options("Asc;;Ascending,Desc;;Descending",$_REQUEST['dbf_sort_direction']) . "</select>\n";
			$EXPORT['sort_selector'] .= " <select name=\"dbf_sort_direction\" class=\"page_filter_select\">" . csv_to_options("Asc,Desc",$this->filter_values['dbf_sort_direction']) . "</select>\n";
		}
		//$output_text .= "</div>\n";
	}
...
<div class="collection">
<div class="item_value_top"><select name="dbf_rpp">' . records_per_page_options($rpp_currently_selected) . '</select></div><div class="item_value_bottom">rows per page<br /><a href="Javascript:dbf_displayfields_popup_open(\''.$this->value('recordset_source').'\',\''. $this->options['filter']['filter_id'] . '\',\'' . $dyn_fields_to_display_list . '\')">set display fields</a></div>
</div>
<input type="hidden" name="dbf_cur_recordset_start" value="' . $cur_recordset_start . '"><input type="hidden" name="dbf_rpp_pre" value="' . $rpp_currently_selected . '">
		...
		$EXPORT['records_per_page_select'] .= "class=\"page_filter_select\">" . records_per_page_options($rpp_currently_selected) . "</select><input type=\"hidden\" name=\"dbf_cur_recordset_start\" value=\"" . $cur_recordset_start . "\"><input type=\"hidden\" name=\"dbf_rpp_pre\" value=\"" . $rpp_currently_selected . "\">\n";
		$EXPORT['records_per_page_full'] = "Display <select name=\"dbf_rpp\" class=\"page_filter_select\">" . records_per_page_options($rpp_currently_selected) . "</select> Records Per Page<input type=\"hidden\" name=\"dbf_cur_recordset_start\" value=\"" . $cur_recordset_start . "\"><input type=\"hidden\" name=\"dbf_rpp_pre\" value=\"" . $rpp_currently_selected . "\">\n";
...
<div class="filter_image_div"><input type="submit" value="Apply" onClick="Javascript:display_items()" style="width:40px; height:20px; background-color:#ffffff; color:#1b2c67; font-family:Tahoma, Trebuchet MS; font-size:10px;">';
	...
	$output_text .= "<div align=\"center\" id=\"dateSearchPopup\" style=\"display:$dbf_date_display;\"><b>Filter Between Dates:</b> " . date_input_field($this->value("start_dbf_search_date"),"","dbf_search_date_start") . " - " . date_input_field($this->value("end_dbf_search_date"),"","dbf_search_date_end","") . "<input type=\"hidden\" name=\"active_date_filter\" value=\"$dbf_date_active\"><input type=\"hidden\" name=\"dbf_date_filter_field\" value=\"";
	...
	if ($this->options['filter']['dbf_drop_down_filter_field']){
		...
		foreach ($all_drop_down_filter_fields as $dbf_drop_down_filter_field){
			...
			$drop_down_options=csv_to_options(get_sql_list_values($drop_down_sql),$this->filter_values[$element_id_name]);
			$EXPORT['drop_down_filter'] = "<select id=\"$element_id_name\" name=\"$element_id_name\" onChange=\"search_data()\" class=\"page_filter_select\"><option value=\"\">All Types</option>$drop_down_options</select>";
...
function set_fields_to_display(){
...
function recordset_sql(){
	...
	if ($this->options['filter']['dbf_data_filter_value'] && !$this->options['filter']['field_equals']){
        ...
        if ($this->options['filter']['field_equals']){
                $temp = preg_match("/( IN |=|>|<)/",$this->options['filter']['field_equals'],$fe_matches);
                $fe_operator = $fe_matches[0][0];
		if (!$fe_operator || $fe_operator == " "){ $fe_operator = $fe_matches[0]; }
                $field_equals = preg_split("/( IN |=|>|<)/",$this->options['filter']['field_equals']);
	...
	// MATT the filter_field_between_dates is the all important one below that stops the crash. Need to look more closely at the rest if still problems..
        ...
        if ($this->filter_values['active_az_filter']){
                ...
                if (preg_match("/SQL:/",$this->options['filter'][$this->options['filter']['search_a-z']]['select_value_list'])){
                        $possible_az_values=search_linked_field($this->options['filter'][$this->options['filter']['search_a-z']]['select_value_list'],$dbf_az_filter_value,"startOfString");
	...
	if ($this->options['filter']['sql_filter']){
		...
		foreach ($matches as $each_match){
			...
			if (strlen(strpos($each_match_var,"SQL:"))){ // if it is an sql query
				...
				$filter_result=$db->query($sql_filter_query) or die ("<b>Problem with $sql_filter_query:</b><p>" . ->db_error());
		...
		if (!$matches){
			...
			$this->options['filter']['sql_filter']=eval_request($this->options['filter']['sql_filter']);
	...
	if (($this->filter_values["dbf_search_for"] || strlen($this->filter_values["dbf_search_for"])) || $this->options['dbf_search_for']){
		...
		if ($this->filter_values['dbf_search_fields']){$fields_to_search=$this->filter_values['dbf_search_fields'];} else if ($this->options['filter']['dbf_search_fields']){$fields_to_search=$this->options['filter']['dbf_search_fields'];}
		...
		if ($fields_to_search){
			...
			foreach ($each_search_fields as $each_search_field){
				...
				if (preg_match("/SQL: ?SELECT \w+ ?, ?\w+/i",$this->options['filter'][$each_search_field]['select_value_list'])){
					...
					$possible_values=search_linked_field($this->options['filter'][$each_search_field]['select_value_list'],$this->options['dbf_search_for'],"all",$specify_search_link_table);
					$prepend_fieldnames="";
	...
	if ($_REQUEST['dbf_drop_down_filter-1'] && $this->options['filter']['dbf_drop_down_filter_field']){
		array_push($where_clauses, $dd_filter_fields[0] . " = " . $_REQUEST['dbf_drop_down_filter-1']);	
	}
	if ($_REQUEST['dbf_drop_down_filter-2'] && $this->options['filter']['dbf_drop_down_filter_field']){
		array_push($where_clauses, $dd_filter_fields[1] . " = " . $_REQUEST['dbf_drop_down_filter-2']);	
	}
	if ($_REQUEST['dbf_drop_down_filter-3'] && $this->options['filter']['dbf_drop_down_filter_field']){
		array_push($where_clauses, $dd_filter_fields[2] . " = " . $_REQUEST['dbf_drop_down_filter-3']);	
	}
	if ($_REQUEST['dbf_drop_down_filter-4'] && $this->options['filter']['dbf_drop_down_filter_field']){
		array_push($where_clauses, $dd_filter_fields[3] . " = " . $_REQUEST['dbf_drop_down_filter-4']);	
	}
	if ($_REQUEST['dbf_drop_down_filter-5'] && $this->options['filter']['dbf_drop_down_filter_field']){
		array_push($where_clauses, $dd_filter_fields[4] . " = " . $_REQUEST['dbf_drop_down_filter-5']);	
	}
...
function add_limits_to_sql($sql){
...
function check_permissions_on_returned_records($permissions_result,$result){
...
function paging_and_sub_display($result,$total_results_for_non_limited_query){
	...
	if ($this->value("display_end")==0){
			if ($this->options['filter']['no_results_message']){
				...
				$output_text .= "<table border=0 cellpadding=0 cellspacing=0><tr><td valign=\"middle\"><img src=\"".SYSIMGPATH."/icons/exclamation.png\" /></td><td valign=\"middle\">&nbsp; <font size='1'>No Data Found that matched your search criterea.</font></td></tr></table>";
...
function table_header_row(){
	...
	if ($this->value("on_query")){$fields_to_display_array=explode(",",queryfields_as_csv_taking_care_of_brackets($this->options['filter']['display_fields']));}
...
function column_totals($total_cols){
	$output_text = "<tr>";
...
function column_totals_report_at_base($total_cols){
...
function data_to_excel_spreadsheet($data,$tablename,$options){
	global $libpath;
	require_once("$libpath/excel.php"); // load excel writing functions
...
function moodgets_data_store($data,$source,$total_results,$options){
...
function data_to_moodgets_grid($data,$source,$options){
	$headings=array();
	foreach ($data as $row){
		foreach ($row as $column => $value){
...
function print_rows($result,$fields_to_display,$total_cols){
	...
	while ($row=mysql_fetch_array($result,MYSQL_ASSOC)){
		...
		foreach ($row as $row_variable => $row_value){
			...
			if (strpos($check_fields_to_display,$check_row_variable)>0){
				...
				if ($this->options['filter']['hyperlink_field']=="all" || $this->options['filter']['hyperlink_field']==$row_variable){
					$link_to="";
...
if ($user->value("id")==1){
        //print "<p style=\"color:red\">key stuff for rv '$row_variable' is" . $this->options['filter'][$row_variable]['select_key_table_for_record_lists'] . "</p>";
							...
							$EXPORT['rows'][$row[$pk]][$row_variable]=sql_value_from_id($this->options['filter'][$row_variable]['select_value_list'],$row_value,$this->options['filter'][$row_variable]['select_key_table_for_record_lists']);
						...
						$EXPORT['rows'][$row[$pk]][$row_variable]=sql_value_from_id($this->options['filter'][$row_variable]['select_value_list'],$row_value);
						$output_text .= $EXPORT['rows'][$row[$pk]][$row_variable];
							...
							if ($rebuild_svl){ $this->options['filter'][$row_variable]['select_value_list']=$rebuild_svl;}
  							if ($debug){
								print "<p>Now calling sql_value_from_id on " . $row_value . " and " . $this->options['filter'][$row_variable]['select_value_list']; }
 							$output_text .= sql_value_from_id($this->options['filter'][$row_variable]['select_value_list'],$row_value);
    						} else if (strlen(strpos($this->options['filter'][$row_variable]['select_value_list'],"CODE:"))) {
							$output_text .= select_code_value_from_id($this->options['filter'][$row_variable]['select_value_list'],$row_value);
						}else {
							if ($link_to){
		...
		if ($_REQUEST['relation_id'] && $_REQUEST['relation_key']){ $add_relation_link = "&relation_key=".$_REQUEST['relation_key']."&relation_id=".$_REQUEST['relation_id'];}
...
function print_rows_from_custom_search($results,$fields_to_display,$total_cols){
...
function build_search_criterea_for_multiple_words($each_search_field){
