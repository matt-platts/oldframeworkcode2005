...
class shopping_cart {
	...
	function __construct(){
		...
		while ($row=$db->fetch_array($res)){
			...
			$this->generate_order_number=$row['generate_order_number'];
			$this->order_number_template=$row['order_number_template'];
...
public function view_cart_general($cart_view_options){
	...
	$cart_header="";
	if ($_SESSION['order_number_set'] && !$_SESSION['allow_add_once']){ $cart_view_options['allow_update']=0;}
	...
	foreach ($_SESSION['cart'] as $item => $itemdata){
		...
		$sql_select_list=$products_pk . ",".$this->value("product_title_fields").",".$this->price_field.",image,is_download";
		...
		if (($qty_in_stock=="OutOfStock" || $qty_in_stock=="Unknown") && !$nh['is_download'] && $this->value("check_stock_at_checkout")){
			...
			if ($this->value("email_when_back_in_stock")){
				$cart_data[$cart_lines_counter]['stock_alert'] .= "<a href=\"site.php?action=add_product_to_email_list&amp;product=".$item."&mt=31&s=3\" rel=\"#add_to_email_list_overlay\">Email me when this item is back in stock</a>";
			}
...
//                $return .= "<tr><td></td><td></td><td><a href=\"Javascript:document.forms['cart'].submit()\" class=\"order_button\" style=\"text-align:center\">Update</a></td><td></td><td class=\"inline_cart_total\">".$this->value("default_currency_symbol").sprintf("%4.2f",$running_total)."</td><td></td></tr>";
	...
	if ($cart_view_options['run_modules']){
		...
		$return .= "<tr><td colspan=\"4\" align=\"right\" valign=\"top\"><b>Shipping: </b></td><td align=\"right\">" . $EXPORT['shipping_total'] . "</td></tr>"; 
		...
		foreach ($itemise_checkout_modules as $checkout_module_name=>$checkout_module_data){
			if ($checkout_module_name != "amount_to_add_to_total"){
				...
				$return .= "<tr><td colspan=\"4\" align=\"right\"><b>" . $module_text .":</b></td><td align=\"right\">" . $this->value("default_currency_symbol") . $checkout_module_data['total'] . "</td>";
	...
	require_once(LIBPATH . "/classes/core/recordset.php");
...
public function confirm_order($account_code_validated){
	...
	if ((!$_COOKIE['login'] || !$user->value("id")) && $this->value("buy_requires_login")==1){
		// THIS SHOULD BE TEMPLATED BELOW!!!
	...
	} else if (($this->value("buy_requires_login")==2 || $this->value("buy_requires_login")==0) && !$_COOKIE['login']){
		$_SESSION['checkout_without_login']=1;
...
public function place_order(){
	...
	//if ($_SESSION['cart']){ // this line copied in from /lib directory. Not sure if was removed deliberately or added to /lib later?
	...
	if (!$this->value("buy_requires_login") || $_SESSION['checkout_without_login']){
		...
		$order_user_data_sql .= "\")";
		$order_user_res = $db->query($order_user_data_sql) or format_error("Cannot add user data to database. $order_user_data_sql " . $db->db_error(),1,"","SQL:$order_user_data_sql<br />Message: " . $db->db_error());
	...
	$order_res = $db->query($order_sql) or $cart_error = $this->display_mysql_cart_error($order_sql . " gave " . $db->db_error());
	$order_id = $db->last_insert_id();

	// alternate order number - not by the primary key
	$generate_alternate_order_number=0;
	if ($this->value('generate_order_number')){
		$order_number="U" . $order_id;
	} else {
		$order_number=$order_id;
	}
	$order_number_sql="UPDATE orders SET order_number = " . $order_id . " WHERE id = $order_id";
	$order_number_result = $db->query($order_number_sql);
	...
	foreach ($_SESSION['cart'] as $item => $itemdata){
		...
		$product_res=$db->query($product_sql) or $cart_error = $this->display_mysql_cart_error($db->db_error());
		...
		if ($_SESSION['cart'][$item]['attributes']){
			foreach ($_SESSION['cart'][$item]['attributes'] as $attributename => $attributevalue){
				...
				$attr_res = $db->query($attr_sql) or $cart_error = $this->display_mysql_cart_error($db->db_error());
			}
		}
	}
...
public function place_pre_orders(){
	...
	foreach ($_SESSION['preorder_cart'] as $item => $itemdata){
		...
		$this_preorder_total_amount=$individual_item_price*$_SESSION['preorder_cart'][$item]['quantity']; // the price of the preorder on its own
...
function log_order_pre_payment(){
	...
	if ($_SESSION['PayPreordersNow']){
		// order has already beenb logged?
		$_POST['payment_method']=$_SESSION['payment_method_in_use'];
		$place_order_result=1;
	} else {
...
function complete_order_after_payment_taken($order_number){
	...
	if ($this->value("external_order_post_function")){
		...
		//$build_function=str_replace("{=order_number}",$internalSaleId);
		$build_function=str_replace("{=order_number}",$internalSaleId,$build_function);
	...
	if (!$order_number){ // order number supplied means going through the new sagepay wrapper
		...
		unset($_SESSION['PayPreordersNow']); 
	}
...
function complete_order_after_payment_taken_new($order_number,$preorder){
	...
	if ($this->value("external_order_post_function")){
		list($post_module,$post_function)=explode("::",$this->external_order_post_function);
		require_once("$libpath/classes/external/$post_module.php");
		$order_post_module=new $post_module;
		$build_function = "\$order_post_result = \$order_post_module->" . $post_function .= ";";
		//$build_function=str_replace("{=order_number}",$internalSaleId);
		$build_function=str_replace("{=order_number}",$internalSaleId,$build_function);
...
function mail_final_order_confirmation_to_customer($order_number){
	...
	if (!$this->value("buy_requires_login") || $this->value("buy_requires_login")==2 && $_SESSION['checkout_without_login']){
		...
		$res1=$db->query($order_user_sql) or format_error("Error 81871C",1,"","SQL: $order_user_sql<br />Error: " . $db->db_error());
...
function mail_final_order_confirmation_to_office(){
	...
	if (!$this->value("buy_requires_login") || $this->value("buy_requires_login")==2 && $_SESSION['checkout_without_login']){
		...
		//$order_user_sql="SELECT * from orders INNER JOIN order_user_data ON orders.ordered_by=order_user_data.id WHERE orders.id = $internalSaleId";
		$order_user_sql="SELECT * from orders INNER JOIN order_user_data ON orders.id=order_user_data.order_id WHERE orders.id = $internalSaleId";
		$res1=$db->query($order_user_sql) or format_error("Error 81871O",1,"","SQL: $order_user_sql<br />Error: " . $db->db_error());
		$total_rows=$db->num_rows($res1);
		if (!$total_rows){
			print "Error: no rows returned from query of $order_user_sql !!!!";
		}
...
function create_download_urls(){
	...
	while ($h=$db->fetch_array($res)){
		...
		$insert_url_res=$db->query($insert_download_sql) or die("Cant run $insert_download_sql: " . $db->db_error());

		// get product title
		    $get_title_sql="SELECT " . $this->value("product_title_fields") . " FROM products WHERE id = " . $h['product_id'];
		    $title_res=$db->query($get_title_sql) or die ($db->db_error());
...
function retrieve_download_urls_from_current_order($internalSaleId){
/*rick - added $internalSaleId as a paramater, added test below to get it if it does not exist */
	global $db;
	if (!$internalSaleId){
		$internalSaleId=$this->getInternalSaleId();
	}
...
function get_best_shipping_rate($shipping_options){
	...
	if (!$user->value("id") && ($this->value("buy_requires_login")==1 || $this->value("buy_requires_login")==2)){
		if (!stristr($_SERVER['QUERY_STRING'],"confirm_shipping") && !stristr($_SERVER['QUERY_STRING'],"ion=place_order")) { // will use the default form value
			...
			$best_shipping_quote=" tbc with your address details.";
...
function record_payment(){
	...
	if ($record_payment){
		$sql="INSERT INTO account_payments (user,description,payment_amount,payment_date) VALUES(".$user->value("id").",\"Web Order - Immediate Payment via Credit Card - Order #".$_SESSION['order_id']." \",".$_SESSION['grand_total'].",NOW())";
		$rv=$db->query($sql);
	}
}
...
function pay_for_pre_order($order_id){
	$_SESSION['cart']="";
	unset($_SESSION['cart']);
	$content = "<p><b>Order #$order_id</b></p>";
        global $db;
...
WHERE order_products.order_id = $order_id";
	...
	$content .= $message;
	$content .= '<br /><hr size="1"><p>Click below to continue to the paypal web site to complete your payment:</p><p><a href="/site.php?action=load_payment_module"><img src="https://www.paypal.com/en_US/i/btn/btn_xpressCheckout.gif" align="left" style="margin-right:7px;" border=0></a></p>';
	$content .= "<br clear=\"all\" /><p>Or, <a href=\"checkout.html\">continue to the checkout</a> to pay by credit card.</p>";
	$_SESSION['order_number_set']=$order_id;
	$_SESSION['order_id']=$order_id;
	$_SESSION['payment_method_in_use']=$h['payment_method'];
	$_SESSION['grand_total']=$h['grand_total'];
...
function escape_pre_order_payment(){
	...
	unset($_SESSION['order_number_set']);
	unset($_SESSION['payment_method_in_use']);
	$content = "<p class=\"dbf_para_info\">Your previous pre-order has been cancelled. This item is now in your shopping cart to be placed as a new order.</p>";
...
function user_reauth_preorder($orderno){
	$content="<p>Thank you for your request to reauthorise this order.</p><p>Your request has been placed in a queue and we will respond to you shortly.</p>";
	mail("mattplatts@gmail.com","Preorder Reauth Request","Can you reauthorise order $orderno please?");
	return $content;
}

function set_preorder_preference($to){

	$this->preorder_pay_now=$to;
	if ($to==1){
		$_SESSION['PayPreordersNow']="Yes";
	} else {
		$_SESSION['PayPreordersNow']="No";
	}
	$_SESSION['payment_method_already_set']=$_REQUEST['payment_method'];
	header("Location: /site.php?s=2&action=place_order&mt=1");
	exit;
}
