...
class gift_vouchers_complex{
	...
	function __construct(){
		$this->add_to_total=1;
		$this->add_to_stored_grand_total=0;
		$this->always_display_if_post_text=0;
		$this->itemise_as_part_payment=1;
	}
	...
	function itemise_at_checkout(){
		...
		if ($_SESSION['gift_vouchers_complex_code']){
			...
			$first_sql="SELECT * from voucher_codes WHERE voucher_number = \"". $db->db_escape($_SESSION['gift_vouchers_complex_code']) . "\" AND voucher_codes.valid_from <= NOW() AND voucher_codes.valid_to >= NOW()";
	...
	function process_enter_promotional_code($promo_code){
		...
		// NOTE THAT FREE VOUCHERS CANNOT BE USED - THERE MUST HAVE BEEN A PRODUCT PURCHASED
		global $db;
		$retval=0;
		...
		$sql="SELECT restaurant_voucher_codes.*,products.category AS product_category from restaurant_voucher_codes LEFT JOIN products ON restaurant_voucher_codes.product = products.ID WHERE voucher_number = \"" . $db->db_escape($promo_code) . "\" AND type=\"Purchased\" AND expiry_date >= NOW() AND (used=\"\" OR used IS NULL OR used = 0) AND (voided != 1 OR voided IS NULL) AND product > 0";

		$result=$db->query($sql);
		while ($h = $db->fetch_array($result)){

			// edge cases
			// GIFT VOUCHER product_id CAN NOT by ANOTHER GIFT_VOUCHER product_id UNLESS there is another item in the order
			$gift_voucher_count=0;
			$experience_voucher_count=0;
			foreach ($_SESSION['cart'] AS $pid => $cartitemdata){
				if ($cartitemdata['product_id']==11){
					$gift_voucher_count++;
				}
				// look up the category
				$category = $db->db_quick_match("products","category","ID",$cartitemdata['product_id']);
				if ($category==4){
					$experience_voucher_count++;
				}
			}

			$item_count=count($_SESSION['cart']);
			if ($item_count===$gift_voucher_count){ // all products are gift vouchers - these cannot be purchased with either gift vouchers OR dining experiences
				$retval=0;
				$errmsg="Sorry - Our Gift Vouchers cannot be used for purchasing further gift vouchers." . count($_SESSION['cart']) . "-".$gift_voucher_count;
			} else if (count($_SESSION['cart'])==$gift_voucher_count+$experience_voucher_count && $h['product_category']==4) { // dining vouchers MUST pucrhase at least one non voucher product too..
				$retval=0;
				$errmsg="Sorry - Dining Experience Vouchers cannot be used for purchasing further dining experiences and gift vouchers.";
			} else {
				// not gift vouchers with gift vouchers...
				$retval=1;
				...
				if ($_SESSION['gift_vouchers_complex_type']=="payment"){
					...
					$_SESSION['gift_vouchers_complex']['itemise_as_part_payment']=0;
				}
			}
		}
		...
		if (!$retval){
			...
			while ($h = $db->fetch_array($result)){
				$retval=1;
				$_SESSION['gift_vouchers_complex_code']=$promo_code;
				$_SESSION['gift_vouchers_complex_type']="discount";
				$_SESSION['gift_vouchers_complex']['itemise_as_part_payment']=0;
			}
	...
	function discount_order_by_percentage(){
		...
		$discount_amount="-" . ($_SESSION['total_product_price']/100)*$disc_percentage;
		$_SESSION['voucher_text']=$disc_percentage . "% Off";
		$_SESSION['gift_vouchers_complex_total']=$discount_amount;

		// we may need to discount future vat
		...
		$_SESSION['discount_vat_by']=$remove_vat;
	...
	function discount_order_by_set_amount(){
		global $db;
		$sql="SELECT restaurant_voucher_codes.discount_amount FROM restaurant_voucher_codes WHERE voucher_number = \"" . $db->db_escape($_SESSION['gift_vouchers_complex_code']) . "\" AND (restaurant_voucher_codes.voided != 1 OR restaurant_voucher_codes.voided IS NULL) AND restaurant_voucher_codes.expiry_date >= NOW() AND (restaurant_voucher_codes.used IS NULL or restaurant_voucher_codes.used=\"\" OR restaurant_voucher_codes.used=0)";
		$result=$db->query($sql);
		while ($h = $db->fetch_array($result)){
			$discount_amount=$h['discount_amount'];
		}
		$_SESSION['voucher_text']="&pound;" . $discount_amount. "";
		$_SESSION['gift_vouchers_complex_total']=$discount_amount;
...
Normal reservation booking terms & conditions apply.<br /><br />";
	...
	function set_vouchers_as_used(){
		...
		if ($_SESSION['gift_vouchers_complex_code']){
			global $db;
			global $user;
			$sql = "UPDATE restaurant_voucher_codes SET date_used=NOW(), used=1, redeemed_at=\"Web Order\", used_on_web_order_number = " . $_SESSION['order_id'] . " WHERE voucher_number = \"" . $_SESSION['gift_vouchers_complex_code'] . "\"";
			$rv=$db->query($sql);

			if ($_SESSION['gift_vouchers_complex_type']=="payment"){
				// insert this into the payments table if it is a PAYMENT voucher only
				$sql="INSERT INTO payments (user,description,payment_amount,payment_date,order_number) VALUES(".$user->value("id").",\"Web Order Voucher Payment (Voucher #".$_SESSION['gift_vouchers_complex_code'].") for order #".$_SESSION['order_id']." \",".$_SESSION['gift_vouchers_complex_total'].",NOW(),".$_SESSION['order_id'].")";
				$rv2=$db->query($sql);
			}
	...
	function record_payment(){
        ...
        function post_text(){
			...
			$return="";
                        ...
                        $return="&nbsp;<a href=\"site.php?action=cancel_voucher\"><span style=\"font-size:11px\">Remove Voucher</span></a>";
                }
         //       $return .= " | <a href=\"site.php?action=explain_vat_relief\"<spa style=\"font-size:10px\">Help</span></a>";
                return $return;
        }

	function clear_down_vouchers(){
		...
		unset($_SESSION['discount_vat_by']);
	}
