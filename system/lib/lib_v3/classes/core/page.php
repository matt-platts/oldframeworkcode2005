...
/* CLASS: page
 * Meta: All pages are based on an instance of the page class.
 *
*/
class page {
	...
	protected $view; // the file name for the view in which to show this request 
	...
	function load_page_template($template_id){
		...
		if (array_key_exists("dbf_search_for",$_GET)){
	 ...
	 */
	function load_template_widgets($sent_content,$exported_vars = null){
	...
	*/
	function display_admin_content_by_key_name($key_name,$replace_vars=null){
		global $db;
	...
	*/
	function content_from_id($content_id,$appended_files=0,$replace_vars=0){
		if ($debug){
			print "<p><hr size=1></p>";
		...
		global $db;
		if (array_key_exists("pageEdit",$_GET) && $_GET['pageEdit']){$appended_files=0;}
		...
		*/
		...
		// {=PHP - run php in the 'custom' directory within the 'custom' folder as an include (part of the current environment)
		...
		# {=EXEC - run php externally and outside of the framework environment
		if (strpos($return_content,"{=EXEC:")){
			$split_content=explode("{=EXEC:",$return_content);
			$second_split=explode("}",$split_content[1]);
			$php_script=$second_split[0];
			$php_script = str_replace("/","",$php_script); # don't allow access to scripts in folders other than 'custom'
	...
	function dynamic_seo_lookup($seo_in,$seo_section){
		...
		foreach ($matches[0] as $eachmatch){
			...
			if ($_GET[$pk_val]){
				...
				$res=$db->query($sqlquery) or die($db->db_error());
				$seo_out="";
...
EOT;
	...
	// FOR FRONT END WEB SITE
	public function page_setup($content){
		$current_site = load_web_site_vars($_REQUEST['s']);

		// #1 LOAD HEAD SECTION OF HTML DOCUMENT 
		$jx = $_REQUEST['jx'];
		if (!$jx){
			if ($current_site['default_header']){
				$page_header= $this->load_header($current_site['default_header']);
			} else {
				$page_header= $this->load_header($CONFIG['default_site_header']); // STILL TO BE ADDED - DEFAULTS TABLE?
			}
		...
		if ($this->value("override_template_from_content")){ $template_to_load=$this->page->value("override_template_from_content"); }
		$master_template=$this->load_page_template($template_to_load);

		// #3 if admin type user, and we're not already editing, add the CMS toolbar to the content if its specified in CONFIG
		global $user;
		...
		$this->set_value("page_header",$page_header); 

		$template=str_replace("{=title}",$title,$master_template);
		$template = str_replace("{=content}",$content,$template);

		$this->set_value("content",$this->page_header.$template);
		return $this->page_header . $template;
