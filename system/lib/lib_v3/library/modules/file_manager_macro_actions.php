...
/* 
File: file_manager_macro_actions.php
*/
...
// this function is called by the macro action only..
	...
	if (preg_match("/.jpg$/i",$original_file)){
		$return_sizes = resize_image($original_file,$options);
	} else if (preg_match("/.png$/i",$original_file)){
		$return_sizes = resize_png_image($original_file,$options);
	} else {
		return "Source image must be a jpg or png - sorry but $original_file cannot be resized. Please upload a .jpg or .png format image instead.";
	}
