...
function checkForm(){
	...
	if (!document.forms[0].elements['new_cost_centre_no'].value){formError=1; alert("You must enter a Cost Centre Number");}
	if (!document.forms[0].elements['new_delivery_address'].value){formError=1; alert("You must select an office as a delivery address");}

	if (!formError){ document.forms[0].submit();}
	return;
...
while ($h=mysql_fetch_array($res,MYSQL_ASSOC)){
	...
	while ($h1=mysql_fetch_array($res2,MYSQL_ASSOC)){
		array_push($productsarray[$sectionscount],$h1['id']."|".$h1['item_number'] . "|" . $h1['description']."|".$h1['max_order_quantity']."|".$h1['unit_of_issue']."|".$h1['price_per_issue']."|".$h1['pdf']);
		array_push($products_temp_array,"".$h1['id']."|".$h1['item_number'] . "|" . $h1['description']."|".$h1['max_order_quantity']."|".$h1['unit_of_issue']."|".$h1['price_per_issue']."|".$h1['pdf']);
	}
...
infoSpanToUpdate=fieldtoupdate;
if (infoSpanToUpdate=="select_1"){
	infoSpanToUpdate="item_1_info";
} else {
	infoSpanToUpdate=infoSpanToUpdate.replace(/select/,"info");
}
infoSpan(products_matrix[selectedcategory][0].split("|")[0],infoSpanToUpdate);
}
	...
	for (n=0;n<products_matrix.length;n++){
		...
		for (n2=0;n2<products_matrix[n].length;n2++){
			...
			if (productsInner==selectedProduct){
				document.getElementById(spanToUpdate).innerHTML="Unit Of Issue: " + products_matrix[n][n2].split("|")[4] + " | Max Order Quantity: " + products_matrix[n][n2].split("|")[3] + " | PDF: <a href='literature_pdf/" + products_matrix[n][n2].split("|")[6] + "' target=_blank>" + products_matrix[n][n2].split("|")[6] + "</a>";
			}
		}
	}
}
...
<tr><td colspan="2">Delivery Address: <select name="new_delivery_address"><option value="">Please select:</option>
<?php
	global $user;
...
<input type="hidden" name="new_ordered_by" value="<?php echo $user->value("id"); ?>">
