...
if (!$_GET){
	require_once("/var/www/vhosts/dmlmarketing.net/httpdocs/demo_sites/lsmc/library/database.php");
	foreach ($argv as $arg){
		if (preg_match("/=/",$arg)){
			$bits=explode("=",$arg);
			$$bits[0]=$bits[1];
...
// look up video file from id
	$sql="SELECT video from highlights where id = $video_content_id";
	if ($debug){print $sql;}
...
<?php 
	...
	print "Error executing dynamic video code";
}
?>
