...
global $db;
$SQL = "SELECT products.*, artists.artist AS name, DATE_FORMAT(products.release_date,\"%D %M %Y\") AS datetext FROM products INNER JOIN artists ON products.artist=artists.id WHERE release_date > NOW() AND (hidden_on_trade_site IS NULL or hidden_on_trade_site = 0) ORDER BY release_date ASC";
$rv=$db->query($SQL);
